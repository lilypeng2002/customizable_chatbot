...
all_messages = chat_container.empty()
...
with all_messages.container():
    for msg in st.session_state['messages']:
        st.markdown(f"<div class='message {msg['class']}'>{msg['text']}</div>", unsafe_allow_html=True)

input_container = st.empty()  # Reserve space for input container
user_input = input_container.text_input("Type a message...", key="user_input")
...
with col2:
    ...
    if send_pressed:
        ...
        if user_message:  # Ensure there is a message to send
            ...
            input_container.empty() #### HERE NEWWWW
