...
# Send button and input box styling
st.markdown("""
    </div>
</div>
<script>
    document.querySelector('.stTextInput>div>div>input').setAttribute('style', 'width: 85% !important; border-radius: 20px; padding: 10px; margin-right: 10px;');
    document.querySelector('.stButton>button').setAttribute('style', 'width: 15%; border-radius: 20px; padding: 10px;');
</script>
""", unsafe_allow_html=True)


# Functionality to start chat with a bot message if not already done
if not st.session_state['initialized']:
    # Initialize chat with the bot's starting message
    st.session_state['messages'].append({'class': 'bot', 'text': 'Alex: Hey there! I\'m an AI developed by the University of Toronto, and I\'m here to help you explore any desire you may have to become more kind and caring towards others. Can you tell me a little bit more about what\'s been on your mind lately?'})
